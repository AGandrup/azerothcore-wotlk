...
public:
    ...
    [[nodiscard]] bool GetBit(uint32 index) const
    {
...
private:
    [[nodiscard]] static constexpr uint8 CalculateBlockCount(uint32 fieldCount)
    {
        return (fieldCount + CLIENT_UPDATE_MASK_BITS - 1) / CLIENT_UPDATE_MASK_BITS;
    }

    [[nodiscard]] static constexpr std::size_t GetBlockIndex(uint32 bit)
    {
        return bit / 32;
    }

    [[nodiscard]] static constexpr uint32 GetBlockFlag(uint32 bit)
    {
        return 1u << (bit % 32);
    }
